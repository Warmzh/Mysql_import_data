...
namespace sftpUplod
{
    public partial class Form1 : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if ((Int32)DateTime.Now.DayOfWeek == 0 && DateTime.Now.ToString("HH:mm") == "03:40")
            {
       ...
       AND (   (DATE_FORMAT(swipecardtime, '%Y%m%d') =
        ...
        private void InsertMySql_testemployee_SqlServer(string strBG)
        {
            ...
            if (strBG == "CSBG")
            {
                ...
                String sql = "SELECT zgbh, icid FROM mf_employee where lrzx in ('PQ','MQ','PN')";
                ...
                string SQL = "SELECT emp.EMP_CD, emp.EMP_NAME, emp.DEPT_CD, dept.DEPT_NAME, emp.D_I_CD,'cardid', emp.EXP_DEPT, emp.EMP_STATUS FROM PUB.EMPR emp, PUB.DEPTMENT2 dept WHERE emp.DEPT_CD=dept.dept_cd";
            ...
            else if (strBG == "ASBG")
            {
                ...
                //string SQL = "SELECT EMP_CD, EMP_NAME, DEPT_CD, DEPT_NAME, D_I_CD,'cardid', EXP_DEPT, EMP_STATUS FROM PUB.EMPR";
                string SQL = "SELECT emp.EMP_CD, emp.EMP_NAME, emp.DEPT_CD, dept.DEPT_NAME, emp.D_I_CD,'cardid', emp.EXP_DEPT, emp.EMP_STATUS FROM PUB.EMPR emp, PUB.DEPTMENT2 dept WHERE emp.DEPT_CD=dept.dept_cd";
                //string SQL = "SELECT emp.EMP_CD EMP_CD, emp.EMP_NAME EMP_NAME, emp.DEPT_CD DEPT_CD, dept.DEPT_NAME DEPT_NAME, emp.D_I_CD D_I_CD,'cardid', emp.EXP_DEPT EXP_DEPT, emp.EMP_STATUS EMP_STATUS FROM PUB.EMPR emp, PUB.DEPTMENT2 dept WHERE emp.DEPT_CD=dept.dept_cd";
        ...
        private void InsertMysql_empClassNull(string strBG)
        {
            ...
            {   //获取当前日期对应的周六的班别
                ...
                DataTable DT5 = mySqlHelper.QueryDBC(sql);
                //获取当前日期对应得周日班别
                 ...
                 * try
                 {
                     ...
                     foreach (DataRow dr5 in DT5.Rows)
                     {
                         strSQLUpdate = "update swipecard.emp_class set class_no='" + dr5["class_no"].ToString() + "' where emp_date=DATE_FORMAT(subdate(curdate(),date_format(curdate(),'%w')),'%Y-%m-%d') and class_no='502' and id='" + dr5["id"].ToString() + "';";
                         strSqls.Add(strSQLUpdate);
                     }
            ...
            else if (strBG == "ASBG")
            {
                ...
                try
                {
                    DataTable DT5 = mySqlHelper.QueryDbAsbg(sql);
                    //删除周日为502的班别
                    string strSQL = "delete from swipecard.emp_class where  emp_date=DATE_FORMAT(subdate(curdate(),date_format(curdate(),'%w')),'%Y-%m-%d') and class_no='502'";
                    //WriteLog(DateTime.Now + "-->delete:" + " " + '\r', 1);
                    cmd.CommandText = strSQL;
                    ...
                    cmd.ExecuteNonQuery();
                    tx.Commit();
                    WriteLog("-->insert153_emp_class:" + UpdateSumk + '\r', 1);
                }
                catch (Exception ex)
                {
                    tx.Rollback();
                    WriteLog("-->insert153_emp_class,Rollback :" + ex.Message + '\r', 2);
                }
                finally
                {
                    tx.Dispose();
        ...
        private void InsertMySql_empClass0330(string strBG)
        {
            ...
            if (strBG == "CSBG")
            {
                ...
                try
                {
                    ...
                    //    string sql = "select id,class_no from emp_class where DAYOFWEEK(emp_date)=7 and id= '" + dr4[0].ToString() + "'";
                    //    DataTable DT4 = MySqlHelp.QueryDBC(sql);
                    //    foreach (DataRow dr5 in DT4.Rows) {
                    //        strSQLUpdate = "update swipecard.emp_class set class_no='"+dr4[1].ToString()+"' where emp_date>=curdate() and emp_date<date_add(CURDATE(),interval 2 day) and class_no=''and id='"+dr5[0].ToString()+"'";
